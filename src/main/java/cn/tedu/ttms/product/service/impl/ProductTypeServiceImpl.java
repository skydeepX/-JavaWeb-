...
/**
 * spring中声明式事务处理,假如是基于注解方式需要借助@Transactional注解,此注解可以写在
 * 类上,也可以写在方法上.当某个业务类上使用了@Transaction 注解时,Spring默认就会通过
 * AOP为此类创建代理对象,然后通多代理对象为业务方法织入事务处理功能.
 */
@Transactional
@Service
public class ProductTypeServiceImpl implements ProductTypeService {
	...
	public void saveObject(ProductType entity) {
		if(entity==null)	throw new ServiceException("保存的对象不能为空");
